...
opt.experiment += ('_' + opt.dataroot1.split('/')[0] + '_Adv' )
...
labelBtoAtoB = torch.FloatTensor(opt.batchSize)
labelAtoB = torch.FloatTensor(opt.batchSize)
labelAtoBtoA = torch.FloatTensor(opt.batchSize)
...
if opt.cuda:
    ...
    labelAtoBtoA = labelAtoBtoA.cuda(opt.gpuId)
    labelBtoA = labelBtoA.cuda(opt.gpuId)
    labelBtoAtoB = labelBtoAtoB.cuda(opt.gpuId)
...
labelAtoBtoA = Variable(labelAtoBtoA)
labelBtoA = Variable(labelBtoA)
labelBtoAtoB = Variable(labelBtoAtoB)
...
for epoch in range(0, opt.nepoch):
    while True:
        ...
        cycleB_pred = netD_B(cycleB)
        labelBtoAtoB.data.resize_(cycleB_pred.size() )
        labelBtoAtoB.data.fill_(real)
        lossCycle_B = opt.cycleWeight *criterionCycle(cycleB_pred, labelBtoAtoB)
        ...
        cycleA_pred = netD_A(cycleA)
        labelAtoBtoA.data.resize_(cycleA_pred.size() )
        labelAtoBtoA.data.fill_(real)
        lossCycle_A = opt.cycleWeight * criterionCycle(cycleA_pred, labelAtoBtoA)
        ...
        if j % 20 == 0:
            ...
            print('       D_B: %.4f D_B_real: %.4f D_B_fake: %.4f Cycle_B: %.4f GAN_AtoB: %.4f ' %
                (lossB_D.data[0], lossB_real.data[0], lossB_fake.data[0], lossCycle_B.data[0], lossGAN_AtoB.data[0]) )
        if j%50 == 0:
